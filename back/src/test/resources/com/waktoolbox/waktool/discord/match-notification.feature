Feature: A match notification is sent to the discord channel
  ...
  Scenario: A match notification is sent without a referee
    Given that the current time is {{2024-11-11T15:15:30.000Z}}
    Given the tournaments table will contain:
    """yml
    id: ww
  ...
  Scenario: Notification uses notificationDate when different from date
    Given that the current time is {{2024-11-11T16:15:30.000Z}}
    Given the tournaments table will contain:
    """yml
    id: ww
    content:
    ...
    Given that the current time is {{2024-11-11T16:00:00.000Z}}
  ...
  Scenario Outline: No notification if a match has a missing team
    Given that the current time is {{2024-11-11T15:15:30.000Z}}
    Given the tournaments table will contain:
    """yml
    id: ww
    ...
    Given that posting on "/discord/channels/123/messages" will return a status OK_200
    When the match notificator runs
    Then within 100ms "/discord/channels/123/messages" has received 0 POST

    Examples:
      | TeamA | TeamB | DBTeamA | DBTeamB |
      | T1    |       | T1      | T2      |
      |       | T2    | T1      | T2      |
      | T1    | T2    | No      | T2      |
      | T1    | T2    | T1      | No      |