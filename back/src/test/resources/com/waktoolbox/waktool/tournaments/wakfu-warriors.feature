Feature: Wakfu Warriors workflow
  ...
  Scenario Outline: We can proceed with 127/128 teams
    Given the tournaments table will contain:
  """yml
  id: ww
  content:
  ...
  """
    ...
    # Phased 1 round 1 - First roll - 127/128 teams
    When we start next round or phase of tournament ww
    ...
    """
    # First match for all teams, all random
    And if <numberOfTeams> == 127 => there is 63 pending matches in tournament ww
    And if <numberOfTeams> == 128 => there is 64 pending matches in tournament ww
    Given all teams A of tournament ww win their match
    ...
    # Second match for all teams, 1 loss teams vs 1 loss teams, no loss vs no loss
    Then if <numberOfTeams> == 63 => there is 63 pending matches in tournament ww
    Then if <numberOfTeams> == 64 => there is 64 pending matches in tournament ww
    And the matches drafts are using predefined team breeds in tournament ww phase 1
    Given all teams A of tournament ww win their match
    ...
    # 2 loss are eliminated, 2 wins are qualified, let's fight between 1 loss again
    Then there is 32 pending matches in tournament ww
    And the matches drafts are using predefined team breeds in tournament ww phase 1
    Given all teams A of tournament ww win their match
    ...
    # Phase 3 round 1 - 32 teams
    When we start next round or phase of tournament ww

    # 1 loss teams encounter each other, no loss teams encounter each others
    Then there is 16 pending matches in tournament ww
    And the matches drafts are using predefined team breeds in tournament ww phase 3
    Given all teams A of tournament ww win their match
    ...
    # Phase 6 round 1 - Semi
    When we start next round or phase of tournament ww

    Then there is 2 pending matches in tournament ww
    And the matches drafts are manual in tournament ww phase 6
    Given all teams A of tournament ww win their match
    ...
    Then there is 1 pending matches in tournament ww
    And the matches drafts are manual in tournament ww phase 6 round 3 match round 1
    Given all teams A of tournament ww win their match
